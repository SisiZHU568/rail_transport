...
慢层成本采用唯一的“事件费＋实例—时隙运行费”口径：

- `deployment`：实例真正创建时，按新建实例数一次性计费；复用现有实例不重复收费；
- `cold`：实例进入 `STARTING` 时按新建实例数一次性计费；零冷启动实例仍产生配置定义的创建/初始化成本；
- `running`：在故障处理和慢层计划原子提交完成后，按资源服务区间 \([t\Delta t,(t+1)\Delta t)\) 内实际存在的 `STARTING + WARM` 实例收取实例—时隙费；在区间起点新建的实例当期计费，在区间起点已因故障销毁的实例当期不计费；
...
- 快照过期或非法计划原子拒绝；
- 实例数只能取 0 或允许的正整数；
- 部署、冷启动和实例—时隙运行成本计费时点唯一，保留承诺不重复收费；
- 配置和候选中不存在车载计算节点。
...
实例数评分在安全候选集合中确定性量化。保留时间配置使用秒单位的 \(L^{min}_{k,i}>0\)、\(L^{step}_{k,i}>0\)、\(L^{max}_{k,i}\ge L^{min}_{k,i}\)。唯一的整数时隙档位集合为：
...
各最大项使用与 §4.6 成本账本及快层计费完全相同的函数和价格，只把实例数、功率、处理时间、链路 bit 和云用量替换成配置上限。不存在独立货币 `retention` 项；`running` 上界包含整个慢帧内启动中和温实例的最大实例—时隙数。其中计算能耗采用确定性安全上界
...
对计划 \(a\) 的目标节点，定义慢帧 CPU 周期容量及对应的输入等效 bit 容量：

\[
cap^W_{k,i}=n_{k,i}F_{k,i}^{inst,max}H,
\qquad
cap^{eq}_{k,i}=cap^W_{k,i}/(C_k\gamma_k).
\]

代理工作量按固定的“容量—路径运输”规则逐阶段传播，而不是把每个 VNF 都视为直接从服务 MEC 输入。

第一阶段的源分布由当前/下一服务 MEC 各占预测新到达量的一半，再加当前第一阶段队列和将在本慢帧到达第一阶段的在途量。对阶段 \(k>1\)，源节点 \(h\) 的供应量等于上一阶段分配到 \(h\) 的输入等效工作量；同节点运输成本为零，跨节点运输成本为当前正容量最短路径的逐链路 bit 费用。若源与某目标不可达，不生成运输边。

对每一阶段，先给目标节点一个容量吸引分数：

\[
s_{k,i}=cap^{eq}_{k,i}/(1+\bar d_{k,i}/d_{scale}),
\qquad
\widehat B_{k,i}^{eq,target}=\widehat B_k^{eq}
\frac{s_{k,i}}{\sum_js_{k,j}},
\]

其中 \(\bar d_{k,i}\) 是本阶段源分布到目标 \(i\) 的供应量加权最短路径距离。随后按固定源节点顺序和目标键
`(unit_path_cost, target_node_id)`，将每个源的输入等效 bit 供应量依次运输到尚有目标需求的节点；相同键按节点 ID 决定。得到输入等效运输量 \(\widehat y^{eq}_{k,h,i}\)，并令：

\[
\widehat B^{eq}_{k,i}=\sum_h\widehat y^{eq}_{k,h,i},
\qquad
\widehat W_{k,i}=C_k\gamma_k\widehat B^{eq}_{k,i},
\qquad
...
这是确定性的容量目标＋贪心最小单位费用运输规则，不声称求解未来最优流。只有 VNF 1 使用服务 MEC 作为新到达源；VNF \(k>1\) 严格使用上一阶段代理分布。不可达目标份额为零；若供应无法完整运输，则代理目标为正无穷并排在所有可评估计划之后。
...
其中 \(\widehat C^{wired}=\sum_k\widehat C_k^{wired}\) 来自上述逐阶段运输，\(\widehat C^{cloud}\) 按分配到云的物理 bit 和 CPU 周期使用配置云价格计算。\(\widehat C^{UL}\) 使用预测上行量、当前 \(h_t\)、全上行带宽 \(B^{UL}\) 和时长 \(H\) 反解满足香农约束的最小平均功率：
...
教师剪枝除安全解码的内存/故障域剪枝外，还使用目标下界：

- 成本教师：已选模式已发生的不可逆成本，加上每个剩余 VNF 的最小合法新增/运行/可达运输成本；
- 可靠性教师：先用剩余 VNF 可达到的最大结构可靠性裕度构造 \(-\Delta R_T^{struct}\) 的乐观下界，可靠性并列后再使用成本下界；
- 均衡教师：当前节点利用率最大值与剩余 VNF 最小可能负载增量给出最大利用率下界，再使用方差非负下界和成本下界。
...
\[
\min_{a\in F^{safe}(s)}
(J_{cost}(a),-\Delta R^{struct}_T(a),J_{balance}(a),key(a)).
\]
...
\[
\min_{a\in F^{safe}(s)}
(\max_i u_i(a),Var_i[u_i(a)],J_{cost}(a),-\Delta R^{struct}_T(a),key(a)).
\]
...
节点对集合明确定义为：对每个 VNF \(k\)，教师计划中目标实例数大于零的允许节点为正节点，目标实例数为零的允许节点为负节点；\(E\) 是同一 VNF 内所有 `(正节点, 负节点)` 有序对的并集。若 \(|E|=0\)，定义 \(L_{rank}=0\)，不得执行空集合除法。
...
该状态没有新的 `DeploymentPlan`，环境保持仍存活的现有实例。轨迹仍保存策略产生的完整原始评分和完整联合旧对数概率；`effective_action_mask` 固定为：所有实例数维度有效、所有保留时间维度无效。该掩码只用于环境语义、统计和保留辅助损失，PPO 概率比与 KL 仍使用完整动作维度。
