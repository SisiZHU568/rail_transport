...
6. 队列层原子扫描所有目标绑定片段：当目标节点失效或对应 VNF 已无健康温实例时清除 `routing_target_node`；片段位置和可用时隙不变，不产生免费转发；
7. 新业务批次进入上行队列；
...
三类教师共用一个完全因果的单慢帧代理评估器。令慢帧时长 \(H=N_{slow}\Delta t\)，`ObservationSpec` 中固定的到达率 EWMA 为 \(\hat\lambda_s\)。下一慢帧的新到达原始输入等效量代理为：

\[
\widehat A_{new}^{eq}=H\sum_s\hat\lambda_s.
\]
...
预演输出每个快时隙的温实例数与活动实例数。定义温实例可用时间和慢帧 CPU 周期容量：

\[
 A^{warm}_{k,i,T}
=\sum_{t\in T}m^{warm}_{k,i,t}\Delta t,
\qquad
cap^W_{k,i,T}=F_{k,i}^{inst,max}A^{warm}_{k,i,T},
\qquad
cap^{eq}_{k,i,T}=cap^W_{k,i,T}/(C_k\gamma_k).
\]
...
对每一阶段，先给目标节点一个容量吸引分数：

\[
s_{k,i}=cap^{eq}_{k,i,T}/(1+\bar d_{k,i}/d_{scale}),
\qquad
\widehat B_{k,i}^{eq,target}=\widehat B_k^{eq,available}
\frac{s_{k,i}}{\sum_js_{k,j}},
\]

其中 \(\widehat B_k^{eq,available}=\sum_iS_{k,i}^{eq}\)，\(\bar d_{k,i}\) 是本阶段源分布到目标 \(i\) 的供应量加权最短路径距离。若 \(\sum_js_{k,j}=0\)，该阶段本时隙服务量为零，全部可用供应进入缺口；不得执行零除法。否则，绑定供应优先，未绑定供应随后，并分别按固定源键 `(bound_first, source_node_id, routing_target_node_or_empty)` 和目标键 `(unit_path_cost, target_node_id)` 运输到仍有容量的节点；相同键按实体 ID 决定。每个目标的实际服务量不能超过 \(cap^{eq}_{k,i,T}\)，且节点级总 CPU 周期和核心时间限制也按固定 `(VNF priority, function_id, node_id)` 顺序扣减。得到输入等效运输/服务量 \(\widehat y^{eq}_{k,h,i}\)，并令：

\[
\widehat B^{eq,served}_{k,i}=\sum_h\widehat y^{eq}_{k,h,i},
\qquad
\widehat W_{k,i}=C_k\gamma_k\widehat B^{eq,served}_{k,i},
\qquad
\widehat C_k^{wired}=\sum_{h,i}
path\_bit\_cost_{h,i}\,
\gamma_k\widehat y^{eq}_{k,h,i}.
\]

这是确定性的容量目标＋贪心最小单位费用运输规则，不声称求解未来最优流。只有 VNF 1 使用服务 MEC 作为入口源；VNF \(k>1\) 同时处理其现有队列/在途量和上一阶段代理实际完成量。不可达或容量不足的供应不会使整个计划变为正无穷，而是进入共享预测缺口：

\[
J_{def}(a)=
\underbrace{Q_{UL}^{eq}+\widehat A_{new}^{eq}
-\widehat B_{UL}^{eq,served}}_{\text{上行预测缺口}}
+\sum_k
\left[
\widehat B_k^{eq,available}
-\sum_i\widehat B_{k,i}^{eq,served}(a)
\right]_+.
\]

该缺口使用原始输入等效 bit，表示代理慢帧内未完成的阶段工作量，不等同于真实 SLA 违约。仅结构非法、生命周期预审失败或非有限计算才使计划无效。
...
\[
\widehat\tau_{k,i,t}=\begin{cases}
0,&\widehat W_{k,i,t}=0,\\
\widehat W_{k,i,t}/F_{k,i}^{inst,max},&\widehat W_{k,i,t}>0,
\end{cases}
\]
...
其中 \(\widehat W_{k,i,t}\) 是轻量时隙扫描在时隙 \(t\) 实际分配的 CPU 周期。运输器逐时隙强制 \(\widehat\tau_{k,i,t}\le m^{warm}_{k,i,t}\Delta t\) 以及节点聚合核心时间/CPU 周期上限；超出部分进入 \(J_{def}\)，不再把预测过载变成正无穷。非有限值或生命周期预演不变量破坏才返回 `TEACHER_INTERNAL_FAILURE`。
...
其中 `preview.n_created` 是生命周期只读预演实际生成的新实例数，保证优先复用现有实例且不重复收取部署/冷启动费。\(\widehat C^{wired}=\sum_k\widehat C_k^{wired}\) 来自上述逐阶段运输，\(\widehat C^{cloud}\) 按分配到云的物理 bit 和 CPU 周期使用配置云价格计算。\(\widehat C^{UL}\) 使用预测上行量、当前 \(h_t\)、全上行带宽 \(B^{UL}\) 和时长 \(H\) 反解满足香农约束的最小平均功率：

\[
\widehat p^{UL}=\begin{cases}
0,&\widehat B_{UL}^{eq,served}=0,\\
\dfrac{N_0B^{UL}}{h_t}
\left(2^{\widehat B_{UL}^{eq,served}/(HB^{UL})}-1\right),
&\widehat B_{UL}^{eq,served}>0,
\end{cases}
\qquad
\widehat C^{UL}=\pi^{energy}H\widehat p^{UL}.
\]

无线代理先计算慢帧可上传量 \(\widehat B_{UL}^{eq,served}\)：当 \(h_t\le0\) 时为零，否则由 \(B^{UL}\)、\(P^{max}\) 和 \(H\) 的香农上界截断。只有该已上传量进入 VNF 1 的入口源；未上传的 \(Q_{UL}^{eq}+\widehat A_{new}^{eq}-\widehat B_{UL}^{eq,served}\) 进入 \(J_{def}\)。\(\widehat p^{UL}\) 只反解已服务上传量所需的有限功率并截断于 \(P^{max}\)，因此容量不足或零信道不会使所有教师计划变成正无穷。该无线项对同一状态中的部署计划是共同量，但保留在完整成本报告中。
...
三类教师不允许先完整物化 \(F^{safe}(s)\)。它们复用阶段 D 的安全模式生成器，并分别执行确定性分支定界。搜索顺序、模式顺序和并列键均固定；缓存键还包含教师类型和代理规格哈希。

教师剪枝除安全解码的内存/故障域剪枝外，还使用目标下界。三类教师都首先计算剩余搜索空间可达到的最小预测缺口下界，然后再按各自后续目标计算：

- 成本教师：缺口下界之后，使用已选模式已发生的不可逆成本，加上每个剩余 VNF 的最小合法新增/运行/可达运输成本；
- 可靠性教师：缺口下界之后，用剩余 VNF 可达到的最大结构可靠性裕度构造 \(-\Delta R_T^{struct}\) 的乐观下界，可靠性并列后再使用成本下界；
- 均衡教师：缺口下界之后，用当前节点利用率最大值与剩余 VNF 最小可能负载增量给出最大利用率下界，再使用方差非负下界和成本下界。

只有下界词典序不优于当前最优完整计划时才剪枝。教师搜索使用与解码器分离的确定性上限：
...
成本教师先按规则把所有已部署组合的保留时间固定为最短合法档。虽然单慢帧成本代理中的 `running` 只覆盖当前慢帧，最短保留档仍是其明确的长期成本偏好。随后比较：
...
保留时间规则：成本教师取最短档；可靠性教师取最长档；均衡教师使用上述生命周期预演和有限服务运输器，记录该组合最后一次获得正服务的快时隙，并以从当前慢帧边界到该时隙末的时间作为 \(\widehat t_{clear,k,i}\)；组合没有预测服务时取 0。随后选择不小于该时间的最近合法档，超过最大档时取最大档。这些都是因果预训练启发，不声称未来最优。
...
\(r_{T,j}=\exp(\ell_{T,j}^{new}-\ell_{T,j}^{old})\)，使用从 `clip_ratio_base` 到所选 `clip_ratio` 的指数裁剪调度，再对环境样本、去噪步骤和 minibatch 取均值。这里“完整无界动作概率”是指每个去噪核都包含全部 \(v\) 坐标；不得用 `effective_action_mask` 删除坐标。
